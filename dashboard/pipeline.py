#!/usr/bin/env python3
"""
Fleek Affiliate Dashboard — data + compute pipeline (no HTTP).

The single source of truth for reading Airtable, enriching creators (audience tags,
channels), computing trends / funnel / inspiration, and caching avatars + thumbnails.
Imported by serve.py (local dev server) and the Vercel functions in api/. Stdlib only.
"""
import json
import math
import os
