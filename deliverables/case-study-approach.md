...
- Written back to the record (`Brief`, `Brief URL`, `Brief Generated At`) — the ecosystem stays the source of truth
- Auto-published as a **shareable Notion page** per creator — that's the link they receive
- **Trigger:** stage changes to Onboarded → `scripts/run_brief_job.py` runs. Batchable across the whole cohort. Idempotent: it only touches records whose `Brief` is empty, so a re-run after a crash resumes exactly where it stopped. One click per creator; scales because the input is the record, not fresh human research.

**Inputs (what we actually hold — stated precisely, because a reviewer will ask):**

| Input | What it really is |
|---|---|
| Creator profile | Airtable record: score, strength, weakness, predicted CAC, keywords |
| Audience | Bio + audience fields where captured; absent for many creators, and the brief says so |
| Past videos | 588 real posts scraped via Apify, with view/like/comment counts |
| ~~Transcript~~ **Captions** | TikTok captions and YouTube titles + descriptions. **Not speech transcripts** — we never scraped audio, and the prompt forbids quoting these as spoken words |
| ~~Best Fleek creatives~~ **Partner archetypes** | Fleek's three named top performers (`@behindthesale`, `@theliveneedham`, `@juliacrcl`) as patterns to emulate. There is **no creative asset library** — claiming one would not survive a follow-up question |
| Referral performance | Only exists for creators already carrying a Fleek code. For a new recruit the brief states plainly that this is brief #1 rather than inventing conversion history |
| Trending formats | The cited wiki pages, read at brief time — refresh the wiki, the next brief changes |

**Outputs:** 3 content ideas (each citing the evidence that makes it fit *this* creator), hook, CTA stack, thumbnail direction, talking points, **do-not-mention list**, example captions, posting schedule.

The **do-not-mention list** is the one that earns its place. Fleek's own hardest problem is that pro resellers think Fleek is for beginners. A brief that lets a creator say *"great if you're just starting out"* actively damages the positioning the whole programme depends on. So the model is required to name what must not be said — including any competitor code the creator is currently promoting.

**Deck moment — better than the hypothetical.** Don't use three invented creators. Use three real ones, at three real lifecycle stages, from the actual 49-creator FR roster:

| Creator | Lifecycle | Brief the engine writes |
|---|---|---|
| **Julia Courcelle** | inherited, unmanaged, *already posting Fleek's code* | **re-activation** — the engine detects `RFD-JULIA` in 25 of her 25 scraped videos, reclassifies her from "Prospect / Not started" to an active partner, keeps her real code, and puts the competitor's `JULIA10` on the do-not-mention list |
| **Felix Beauregard** | new recruit, top-scored (82) | **onboarding** — educational, pro-reseller sourcing; brief #1, no conversion history claimed |
| **lina_momo_** | active TikTok, 196k | **always-on** — live-sourcing / Whatnot angle |

Same system, same prompt, three completely different briefs. The differentiation comes out of the creator's own evidence and their lifecycle branch — not from three hand-tuned prompts. **That's AI-native, not mail-merge.**

And the Julia case is the section's strongest line, because the engine *found* it: `python run_campaign.py brief-real juliacourcelle` prints `lifecycle: dormant (inferred: already carries a live Fleek code)`. Fleek's CRM says prospect. Her videos say partner. The engine believes the videos.

**Where this runs (stack strip):** `content_brain/` in the repo. Claude Opus 4.8, structured outputs (the brief comes back as a validated object, never parsed JSON). The trend pack + archetypes sit behind a prompt-cache breakpoint; per-creator evidence goes after it, so the shared prefix is never invalidated. `run_campaign.py brief-real` prints the cache outcome — a receipt, not a promise.

**AI receipts:** evidence in (her real captions, her real code) → profile → brief → the Notion page a creator actually opens.
