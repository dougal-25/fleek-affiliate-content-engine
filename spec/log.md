...
## 2026-07-10 — Section 4 built: the AI brief generator, running on real French creators

The deck described a brief generator the engine could not yet be. Now it can. Full reasoning:
`spec/decisions/2026-07-10-brief-generator-activation.md`.
...
inputs: the creator's Airtable record, their scraped posts and hashtags, the wiki's trend pack.
`notion_publish.py` renders a brief as a shareable page. `scripts/run_brief_job.py` is the
`Stage = Onboarded` job from `spec/autonomy-layer.md` — idempotent, capped at 25 Claude calls per run,
...
Julia Courcelle carries `RFD-JULIA` in **25 of 25** scraped videos while Airtable has her as
`Stage = Prospect / Not started`. She is one of the inherited French partners the brief asks us to activate,
and she promotes a competitor's code in the same description. The engine reclassifies her from the evidence,
keeps her real code, and puts `JULIA10` on the do-not-mention list. **Evidence beats the CRM.**

**Two claims were false and are corrected.** (1) The prompt cache was caching nothing — Opus 4.8 needs a
4096-token prefix and the system prompt is ~250. The trend pack now carries the prefix over the line, and
`llm.cache_report()` measures it. (2) The deck's input list promised transcripts and a Fleek creative
library. We have captions and three named archetypes. `deliverables/case-study-approach.md` §8 now says so.

**Not verified.** No brief has been generated by Claude — the API key returns *credit balance too low*. Notion
publishing has not run live (`NOTION_BRIEFS_PAGE_ID` unset). Airtable writeback *is* verified against the real
base. Until those two fire, Section 4 is built but not proven.
