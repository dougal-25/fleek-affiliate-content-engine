...
|---|---|---|---|
| `brain_refresh` | Perplexity research pass on the Brain's trend questions → dated trends note appended to the Brain | Cron: Mon 06:00 | Repo (markdown, git commit) |
| `discovery` | Apify run seeded with Brain keywords → Claude enrichment → scoring → upsert Prospects | Cron: Mon 07:00 (after brain) | Airtable |
...
1. **LIVE on GitHub Actions cron, with real run logs:** `brain_refresh` + `discovery`. The runs fire tonight so the logs exist before the interview — the autonomy slide shows evidence, not intention.
